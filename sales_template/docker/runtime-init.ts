...
async function main() {
  ...
  console.info('[sales] applying database migrations')
  await migrateDatabase(databaseUrl)

  if (process.env.APP_SKIP_SEED === 'true') {
    console.info('[sales] skipping seed (APP_SKIP_SEED=true)')
    return
  }

  console.info('[sales] seeding catalog and bootstrap data')
  await seedDatabase(databaseUrl)
}

main().catch((error: unknown) => {
  console.error('[sales] startup initialization failed')
