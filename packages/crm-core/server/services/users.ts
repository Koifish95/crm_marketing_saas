...
import { USERNAME_PATTERN, normalizeUsername } from '../../lib/user'
import { normalizeEmail, TEMPORARY_PASSWORD_DEFAULT, isPermanentPasswordCompliant } from '../../lib/password-policy'
...
async function extraRoleIdsByUser(db: AppDatabase, userIds: number[]) {
...
export async function countActiveAdmins(db: AppDatabase, exceptUserId?: number) {
...
async function assertEmailAvailable(db: AppDatabase, email: string, exceptUserId?: number) {
...
async function assertUsernameAvailable(db: AppDatabase, username: string, exceptUserId?: number) {
...
async function uniqueUsername(db: AppDatabase, email: string, exceptUserId?: number) {
...
export async function revokeManagedUserSessions(db: AppDatabase, userId: number) {
  const [row] = await db.select().from(users).where(eq(users.id, userId)).limit(1)
  if (!row) {
    throw new DomainError('User not found.', 404)
  }
...
export async function changeOwnName(db: AppDatabase, userId: number, displayName: string) {
  const [row] = await db.select().from(users).where(eq(users.id, userId)).limit(1)
  if (!row) {
    throw new DomainError('User not found.', 404)
  }
