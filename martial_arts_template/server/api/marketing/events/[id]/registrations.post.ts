...
import { staffEventRegisterSchema } from '../../../../../shared/schemas/event'
import { useDb } from '../../../../database'
import { registerStaffEvent } from '../../../../services/events'
...
export default defineEventHandler(async (event) => {
  ...
  const parsed = staffEventRegisterSchema.safeParse(await readBody(event))
  if (!parsed.success) {
    throw createError({ statusCode: 400, message: parsed.error.issues[0]?.message ?? 'Invalid registration.' })
  }
  try {
    return await registerStaffEvent(useDb(), id, parsed.data, actor)
  } catch (error) {
    throwDomain(error)
  }
})
