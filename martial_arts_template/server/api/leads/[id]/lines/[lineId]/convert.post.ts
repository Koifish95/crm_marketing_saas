...
import { convertLeadLineSchema } from '../../../../../../shared/schemas/lead'
import { useDb } from '../../../../../database'
import { convertLeadLine } from '../../../../../services/conversion'
...
export default defineEventHandler(async (event) => {
  ...
  const parsed = convertLeadLineSchema.safeParse(await readBody(event) ?? {})
  if (!parsed.success) {
    throw createError({ statusCode: 400, message: parsed.error.issues[0]?.message ?? 'Invalid conversion.' })
  }
  try {
    await convertLeadLine(useDb(), lineId, parsed.data, user)
