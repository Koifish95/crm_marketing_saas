...
CRM Core
   ↑
   ├── Martial Arts     (shipped)
   ├── Sales / Software (not started)
   └── Beauty           (not started)
```
...
C1 is **code-shipped** (`packages/crm-core`, workspace, import tests). C2 (Sales proof + CP product catalog) is **not started**. Do not “establish Core” again.
