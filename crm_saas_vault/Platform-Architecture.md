...
CRM Core
   ...
   ├── Sales / Software (C2A Successful; C2B Slice A Successful; sales_template, :5040; no CP catalog)
   └── Beauty           (not started)
```
...
C1 is **code-shipped** (`packages/crm-core`, workspace, import tests). C2A (thin local Sales consumer) is **Successful** at `sales_template/` / `sales-crm` on port 5040. C2B Slice A (Lead/convert/Opportunity workflow) is **Successful**. C2 (Sales vertical **plus** CP product catalog) is **not started** and is **not** authorized by C2A or C2B success. Slice B is the next Sales product-refinement candidate and is **not** authorized. Do not “establish Core” again.
