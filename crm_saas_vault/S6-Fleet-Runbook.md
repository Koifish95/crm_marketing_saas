...
4. **Upgrade** — requires an S6 backup of **this** env. Enter a target image (defaults to current). Rebuilds the local image with `RELEASE_ID`, stamps `EXPECTED_IMAGE` + `RELEASE_ID` in the env file, `--force-recreate` the app container, remounts the same volumes. If the customer has a non-PROD, upgrade that first (Acme lab `:s2` compose is exempt). Workspace shows running `releaseId` and `schemaVersion`. Rollback = Restore + previous image. Do not downgrade SQLite. Map: [[Product-Release-Update-Lifecycle]].
