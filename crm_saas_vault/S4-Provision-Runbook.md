...
Staff unwrap login: `admin` / `setup`, with `mustChangePassword` so the first login opens `/account/password`. The env file stores that bootstrap password. The Control Plane does not store the password they choose afterward.
